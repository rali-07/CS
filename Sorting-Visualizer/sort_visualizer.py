...
def monkey_sort_generator(arr):
    ...
    while attempt < max_attempts:
        ...
        if is_sorted():
            log_message(f"猴子在 {attempt+1} 次尝试后成功！真是一只智慧的猴子。", 'lightgreen')
            break
        attempt += 1
        if attempt % 100 == 0:
            log_message(f"猴子已尝试 {attempt} 次...", '#aaa')
    else:
        log_message(f"猴子尝试超过 {max_attempts} 次，已放弃", 'red')
    yield None
...
def start_sort():
    ...
    if selected_algo == "冒泡排序":
        ...
        if len(data) > 8:
            log_message("猴子挠了挠头，祝你好运", 'orange')
    else:
